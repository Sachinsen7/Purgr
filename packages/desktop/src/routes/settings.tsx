...
const frequencyOptions: readonly ScanFrequency[] = ["manual", "weekly", "monthly"];
...
export default function Settings() {
  ...
  onMount(async () => {
    try {
      ...
      setStatusMessage("Using local defaults until the sidecar is available.");
    }
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatusMessage("Could not save preferences. The backend sidecar may still be offline.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      setStatusMessage("Could not reach Ollama. Check that the local server is running.");
    } finally {
      setIsTesting(false);
    }
  };
  ...
  return (
    <div class="min-h-screen bg-[#111319] text-[#e2e2eb]">
      <aside class="fixed inset-y-0 left-0 hidden w-64 border-r border-white/5 bg-[#111319] px-6 py-6 lg:flex lg:flex-col">
        <div class="mb-10">
          <p class="font-headline text-xl font-extrabold tracking-tight">DevSweep</p>
        ...
        <nav class="space-y-1">
          <For each={navItems}>
            {(item) => (
              ...
              >
                <span class="text-xs uppercase tracking-[0.2em]">{item.label.slice(0, 2)}</span>
          ...
          <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p class="mt-2 max-w-2xl text-sm text-[#bacac5]">
                Configure how DevSweep scans developer storage, talks to local AI, and protects
                directories you never want included in cleanup.
              </p>
            </div>
        ...
        <div class="mx-auto flex max-w-6xl flex-col gap-8 px-6 py-8 lg:px-10">
          ...
          <div class="grid gap-8 xl:grid-cols-[1.35fr_1fr]">
            <section class="space-y-8">
              <section>
                <div class="mb-3 flex items-center gap-2">
                  <span class="text-xs uppercase tracking-[0.25em] text-[#57f1db]">Scan Frequency</span>
                </div>
                      ...
                      {(option) => (
                        <button
                          class={`rounded-2xl px-4 py-4 text-sm font-bold capitalize transition ${
                            settings().scanning.frequency === option
                ...
                <p class="mt-3 px-1 text-xs text-[#bacac5]/70">
                  Background scans stay low priority so local builds and editors remain responsive.
                </p>
              </section>

              <section>
                <div class="mb-3 flex items-center justify-between">
                  <span class="text-xs uppercase tracking-[0.25em] text-[#57f1db]">Excluded Directories</span>
                ...
                <div class="rounded-3xl border border-white/5 bg-[#1e1f26] p-5">
                  <div class="mb-4 flex flex-col gap-3 md:flex-row">
                    <input
                      ...
                      value={pendingPath()}
                      onInput={(event) => setPendingPath(event.currentTarget.value)}
                    />
                    <button
                    ...
                    <For each={settings().exclusions}>
                      {(path) => (
                        <div class="flex items-center justify-between rounded-2xl border border-white/5 bg-[#111319] px-4 py-3">
                          <span class="truncate font-mono text-xs text-[#bacac5]">{path}</span>
                          <button
              ...
              <section class="rounded-[28px] border border-[#ffb4ab]/10 bg-[#300d14]/25 p-6">
                <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p class="font-headline text-xl font-bold text-[#ffb4ab]">Danger Zone</p>
                    <p class="mt-2 max-w-xl text-sm text-[#bacac5]">
                      Clearing history removes saved scan sessions and the audit trail used for
                      previous cleanup reviews.
                    </p>
                  </div>
                  <button
            ...
            <section class="space-y-8">
              <section>
                <div class="mb-3 flex items-center gap-2">
                  <span class="text-xs uppercase tracking-[0.25em] text-[#57f1db]">Active Toolsets</span>
                </div>
                <div class="space-y-3 rounded-3xl border border-white/5 bg-[#1e1f26] p-5">
                  <For each={Object.entries(toolLabels) as [ToolKey, string][]}>
                    {([key, label]) => (
                      <button
                      ...
                      >
                        ...
                        <div
                          class={`relative h-6 w-11 rounded-full transition ${
                            settings().scanning.toolsets[key] ? "bg-[#2dd4bf]" : "bg-[#33343b]"
                          }`}
                        >
                          <span
                            class={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${
                              settings().scanning.toolsets[key] ? "left-6" : "left-1"
                            }`}
                          />
                        </div>
                      </button>
              ...
              <section>
                <div class="mb-3 flex items-center gap-2">
                  <span class="text-xs uppercase tracking-[0.25em] text-[#57f1db]">Local AI Advisor</span>
                </div>
                <div class="space-y-4 rounded-3xl border border-white/5 bg-[#1e1f26] p-5">
                  ...
                  >
                    <div>
                      <p class="text-sm font-semibold text-white">Enable Ollama assistance</p>
                      <p class="mt-1 text-xs text-[#bacac5]/60">
                        Keep recommendations fully local with no cloud provider dependency.
                      </p>
                    </div>
                    <div
                      class={`relative h-6 w-11 rounded-full transition ${
                        settings().ai.enabled ? "bg-[#2dd4bf]" : "bg-[#33343b]"
                      }`}
                    >
                      <span
                        class={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${
                          settings().ai.enabled ? "left-6" : "left-1"
                        }`}
                      />
                    </div>
                  </button>
                  ...
                  <div class="space-y-3">
                    ...
                    <input
                      ...
                      onInput={(event) => updateAIModel(event.currentTarget.value)}
                    />
                  </div>
                  ...
                  <div class="space-y-3">
                    ...
                    <input
                      ...
                      onInput={(event) => updateAIBaseUrl(event.currentTarget.value)}
                    />
                  </div>
                  ...
                  >
                    {isTesting() ? "Testing Ollama..." : "Test Local AI Connection"}
                  </button>
                </div>
              </section>
              ...
              <section>
                <div class="mb-3 flex items-center gap-2">
                  <span class="text-xs uppercase tracking-[0.25em] text-[#57f1db]">System</span>
                </div>
                <div class="grid gap-3 rounded-3xl border border-white/5 bg-[#1e1f26] p-5 sm:grid-cols-3">
                  ...
                  <div class="rounded-2xl bg-[#191b22] p-4">
                    ...
                    <p class="mt-2 text-lg font-semibold text-white">Tauri + FastAPI</p>
                  </div>
                </div>
              </section>
            </section>
