...
export default function Scan() {
  ...
  const activeDirectories = () => {
    const root = currentScan()?.rootPath || rootPath() || "~/Library/Application Support/Code";
    return [
  ...
  return (
    <DesktopShell
      active="scan"
      eyebrow={isScanning() ? "Scanning System..." : "Scan / Configure / Launch"}
          ...
          <div class="mx-auto grid max-w-5xl gap-8 xl:grid-cols-[1.2fr_0.8fr]">
            <section class="rounded-[32px] border border-white/5 bg-[#1e1f26] p-8">
              ...
              <h1 class="mt-3 font-headline text-4xl font-extrabold tracking-tight text-white">
                Point DevSweep at the directories you want analyzed.
              </h1>
              <p class="mt-4 max-w-2xl text-sm leading-6 text-[#bacac5]">
                This route now uses the same Stitch dark-shell styling. Once started, it transitions
                into the live progress screen instead of a plain form card.
              </p>
              ...
              <div class="mt-8 space-y-5">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                  <label class="flex items-center justify-between rounded-2xl border border-white/5 bg-[#191b22] px-4 py-4">
                    <div>
                      <p class="text-sm font-semibold text-white">Include hidden files</p>
                      <p class="mt-1 text-xs text-[#bacac5]/60">Capture caches and dot-directories.</p>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  <div class="rounded-2xl border border-white/5 bg-[#191b22] px-4 py-4">
                    ...
                    <input
                      ...
                      onInput={(event) => {
                        ...
                        setMaxDepth(value ? Number.parseInt(value, 10) : undefined);
                      }}
            ...
            <section class="space-y-6">
                      ...
                      <div class="flex items-center justify-between rounded-2xl bg-[#191b22] px-4 py-3">
                        ...
                        <span
                          class={`font-mono text-[10px] uppercase tracking-[0.18em] ${
                            index() === 0 ? "text-[#57f1db]" : "text-[#9cd1c6]"
                          }`}
                        >
                          {directory.status}
                        </span>
              ...
              <div class="rounded-[32px] border border-white/5 bg-[#1e1f26] p-6">
                ...
                <ul class="mt-4 space-y-3 text-sm text-[#bacac5]">
                  <li>Start with your main workspace or home dev directory.</li>
                  <li>Hidden files reveal the highest-value cache cleanup candidates.</li>
                  <li>Use max depth when you want a faster first pass on huge repos.</li>
                </ul>
              </div>
            </section>
          </div>
          ...
          <div class="mx-auto flex max-w-5xl flex-col items-center justify-center py-8 text-center">
            ...
            <div class="mb-8 w-full max-w-3xl rounded-[28px] border border-white/5 bg-[#0c0e14] p-6 text-left shadow-2xl">
              <div class="mb-4 flex items-center justify-between">
                ...
                <span class="font-mono text-[10px] text-[#57f1db]/60">PID: active</span>
              </div>
            ...
            <div class="mb-10 grid w-full max-w-3xl gap-4 md:grid-cols-2">
              <div class="flex items-center gap-4 rounded-[24px] border border-white/5 bg-[#1e1f26] p-5">
                ...
                <div class="text-left">
                  <p class="font-headline text-3xl font-bold text-white">{scan().scannedFiles}</p>
                  <p class="text-xs text-[#bacac5]/60">Files scanned</p>
                </div>
              </div>
              <div class="flex items-center gap-4 rounded-[24px] border border-white/5 bg-[#1e1f26] p-5">
                ...
                <div class="text-left">
                  <p class="font-headline text-3xl font-bold text-white">
                    {(scan().results.reduce((sum, item) => sum + item.size, 0) / 1024 / 1024 / 1024).toFixed(1)} GB
                  </p>
                  <p class="text-xs text-[#bacac5]/60">Space found</p>
                </div>
              </div>
            </div>
              ...
              <For each={activeDirectories()}>
                {(directory, index) => (
                  <div class="flex items-center justify-between rounded-2xl bg-[#1e1f26] px-4 py-3">
                    <span class="truncate font-mono text-xs text-[#bacac5]">{directory.label}</span>
                    <span
                      class={`font-mono text-[10px] uppercase tracking-[0.18em] ${
                        index() === 0 ? "text-[#57f1db]" : "text-[#9cd1c6]"
                      }`}
